...
- **June 19, 2025 — AlbumentationsX begins.** The first commit in this
  ...
  AlbumentationsX successor repository. Its `LICENSE` stated GNU AGPL version 3
  as the repository default and described a separately available
  commercial-license path. Inherited legacy material retains the MIT notice
  described below.
...
- **Current prospective repository default.** The repository revision that
  introduced this history record adopts `AGPL-3.0-only` prospectively for that
  revision and later revisions unless a later notice says otherwise.
  Separately negotiated commercial licenses may grant alternative permissions
  for the scope stated in an executed agreement or order form. The first
  prospective release is `2.3.2`. The already published `2.3.1` artifacts must
  never be rebuilt or republished with different license metadata.
