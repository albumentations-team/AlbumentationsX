...
def angle_2pi_range(
    ...
    """Decorator that normalizes keypoint angle values (4th column) to [0, 2π) after the wrapped function. Used for keypoint transforms that modify angles.
...
def non_rgb_error(image: np.ndarray) -> None:
    """Raise ValueError if image is not RGB (exactly 3 channels). Used by RGB-only transforms; hints for grayscale and multi-spectral.
...
def check_range(value: tuple[float, float], lower_bound: float, upper_bound: float, name: str | None) -> None:
    """Raise ValueError if value is outside [lower_bound, upper_bound] or if tuple is not (min, max). Used by transform param validation.
...
class PCA:
    """PCA transformer (OpenCV). Fit, transform, inverse_transform, explained variance. Used by color/domain transforms for dimensionality reduction.
    ...
    def fit(self, x: np.ndarray) -> None:
        """Fit the PCA model to the input data (mean, components, explained variance). Call before transform or inverse_transform. Uses OpenCV PCACompute2.
    ...
    def transform(self, x: np.ndarray) -> np.ndarray:
        """Project data onto the principal components (fit must be called first). Centers data with fitted mean, returns (n_samples, n_components). Raises if not fitted.
    ...
    def fit_transform(self, x: np.ndarray) -> np.ndarray:
        """Fit the PCA model and transform the data in one step. Equivalent to fit(x) then transform(x). Returns projected data (n_samples, n_components).
    ...
    def inverse_transform(self, x: np.ndarray) -> np.ndarray:
        """Reconstruct data from PC space to original space. Uses fitted mean and components; lossy if n_components < n_features. Raises if not fitted.
    ...
    def explained_variance_ratio(self) -> np.ndarray:
        """Return fraction of total variance explained by each component (shape (n_components,), sums to <= 1). Raises if not fitted. Useful for choosing n_components.
    ...
    def cumulative_explained_variance_ratio(self) -> np.ndarray:
        """Return cumulative fraction of variance explained (monotonic 0 to <= 1). Useful for choosing n_components to retain e.g. 95% variance. Raises if not fitted.
...
def handle_empty_array(param_name: str) -> Callable[[F], F]:
    """Decorator that returns the empty array immediately when the named array parameter is empty; otherwise calls the wrapped function. Used by bbox/keypoint helpers.
