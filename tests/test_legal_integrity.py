    ...
    *,
    ...
    with tarfile.open(path, "w:gz") as archive:
        ...
        if include_build_output:
            ...
            info = tarfile.TarInfo(f"{root}/legal-dist/albumentationsx-2.3.1-py3-none-any.whl")
...
def test_sdist_rejects_local_build_output(tmp_path: Path) -> None:
    expected_files = _expected_files()
    sdist = tmp_path / f"albumentationsx-{FIRST_ONLY_RELEASE}.tar.gz"
    _write_sdist(sdist, expected_files, include_build_output=True)

    errors = collect_artifact_errors(sdist, expected_files)

    assert errors == [
        f"{sdist.name}: local build-output directory leaked into artifact as "
        f"albumentationsx-{FIRST_ONLY_RELEASE}/legal-dist/albumentationsx-2.3.1-py3-none-any.whl",
    ]
