...
CLA_V2_SHA256 = "e46696db80156d7f50dd017db61f39d903bcd841dcb92a2340dfc6ac909346d7"
...
def _check_project_metadata(repo_root: Path) -> list[str]:
    ...
    required_excludes = ("CLA.md", "legal", "legal-dist", "LICENSES/OFL-1.1.txt")
    errors.extend(
...
def _check_history_and_notices(repo_root: Path) -> list[str]:
    ...
    required_notice_phrases = (
        ...
        "repository-only test assets",
        "excludes both from wheel and sdist artifacts",
...
def _check_cla_archive(repo_root: Path) -> list[str]:
    ...
    provenance = (repo_root / "docs/maintaining/license-provenance.md").read_text()
    for required_value in (
        ...
        if required_value not in manifest:
            errors.append(f"CLA manifest is missing {required_value}")
        if required_value in (CLA_V1_GIST_URL, CLA_V1_GIST_REVISION) and required_value not in provenance:
            errors.append(f"license provenance document is missing hosted CLA identifier {required_value}")
...
def _forbidden_artifact_errors(artifact: Path, members: Mapping[str, bytes]) -> list[str]:
    ...
    for member_name in members:
        ...
        if "legal-dist" in member_path.parts:
            errors.append(f"{artifact.name}: local build-output directory leaked into artifact as {member_name}")
            continue
